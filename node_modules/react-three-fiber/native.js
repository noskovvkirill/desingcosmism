...
import { View, StyleSheet, PixelRatio, PanResponder } from 'react-native';
...
function useCanvas(props) {
  ...
  const handlePointerMove = useCallback((event, prepare = true) => {
    ...
    objects => objects.filter(obj => ['Move', 'Over', 'Enter', 'Out', 'Leave'].some(name => obj.__handlers['pointer' + name]))); // Take care of un-hover
...
const IsReady = /*#__PURE__*/memo(IsReadyComponent);

function CanvasComponent(props) {
  const [size, setSize] = useState(null);
  const [renderer, setRenderer] = useState(); // Handle size changes

  const onLayout = useCallback(function callback(e) {
    const {
      x,
      y,
      width,
  ...
  const setNativeRef = useCallback(function callback(ref) {
...
const Canvas = /*#__PURE__*/memo(CanvasComponent);
